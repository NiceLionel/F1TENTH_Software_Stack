...
class WallFollow(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('distance', 0.0)   # distance to the wall
        ...
        self.declare_parameter('forward_distance', 6.0)   # L in the slides
        self.declare_parameter('steer_angle_threshold', 45)   # steering angle constrain
        self.declare_parameter('plot_switch', "off")   # debug tool, turn it on to see curves ("on" or "off")
        self.declare_parameter('forward_distance_threshold', 2.4)   # threshold of forward distance to help determine if to steer
        self.declare_parameter('angle_clip', 0.03)   # if forward_distance > forward_distance_threshold, clip angle into this range, which means 
                                                     # when there is no wall in the front, too large angle is not allowed
        self.declare_parameter('high_speed', 7.0)   # speed when angle < 15
        self.declare_parameter('medium_speed', 2.4)   # speed when 15 < angle < 30
        self.declare_parameter('low_speed', 1.2)   # speed when angle > 45
    ...
    def get_error(self, range_data, dist):
        ...
        L  = self.get_parameter('forward_distance').get_parameter_value().double_value
        if which_wall == "left":
    ...
    def pid_control(self, forward_distance, error):
        ...
        ac = self.get_parameter('angle_clip').get_parameter_value().double_value
        ...
        if forward_distance > fdt:   # if no wall forward, should never turn to much
            angle = np.clip(angle,-ac,ac)
    ...
    def calculate_speed(self, angle):
        ...
        """
        hs = self.get_parameter('high_speed').get_parameter_value().double_value
        ms = self.get_parameter('medium_speed').get_parameter_value().double_value
        ls = self.get_parameter('low_speed').get_parameter_value().double_value
        if np.abs(angle) < 2 / 180 * np.pi:
            speed = hs
        elif np.abs(angle) >= 2 / 180 * np.pi and np.abs(angle) < 30 / 180 * np.pi:
            speed = ms
        else:
            speed = ls
        return speed
